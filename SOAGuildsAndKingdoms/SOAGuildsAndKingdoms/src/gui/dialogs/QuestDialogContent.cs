...
namespace SOAGuildsAndKingdoms.src.gui.dialogs
{
    public class QuestDialogContent
    {
        ...
        public QuestDialogContent(ICoreClientAPI capi, SOAGuildsAndKingdomsModSystem modSystem,
        ...
        public double AddPeriodTabsToComposer(GuiComposer composer, double yPos, int magicWidth, double xOffset = 0, string? questType = null)
        {
            ...
            if (showWeekly)
            {
                var weeklyLabel = Lang.Get("soaguildsandkingdoms:quests-tab-weekly");
            ...
            if (showMonthly)
            {
                var monthlyLabel = Lang.Get("soaguildsandkingdoms:quests-tab-monthly");
            ...
            if (showSeasonal)
            {
                var seasonalLabel = Lang.Get("soaguildsandkingdoms:quests-tab-seasonal");
        ...
        public double AddQuestContentAsElements(GuiElementContainer container, double startTop)
        {
            ...
            if (isLoading)
            {
                ...
                container.Add(new GuiElementStaticText(capi, Lang.Get("soaguildsandkingdoms:quests-loading"),
        ...
        private double AddNoQuestsMessage(GuiElementContainer container, double yPos, int magicWidth)
        {
            ...
            var message = hasPeriodCompleted && periodType != "weekly"
                ? Lang.Get("soaguildsandkingdoms:quests-period-already-completed")
                : Lang.Get($"soaguildsandkingdoms:quests-none-{periodType}");
        ...
        private double RenderActiveQuestAsElement(GuiElementContainer container, double yPos, int magicWidth, PlayerQuestProgressDto progressDto)
        {
            ...
            container.Add(new GuiElementStaticText(capi, Lang.Get("soaguildsandkingdoms:quests-active"),
            ...
            if (hasIncompleteTurnIn)
            {
                ...
                var submitButton = new GuiElementTextButton(capi, Lang.Get("soaguildsandkingdoms:quests-submit-items"),
            ...
            if (allObjectivesComplete)
            {
                ...
                var claimButton = new GuiElementTextButton(capi, Lang.Get("soaguildsandkingdoms:quests-claim-reward"),
                    CairoFont.WhiteSmallText().WithFontSize(12).WithWeight(Cairo.FontWeight.Bold),
                    CairoFont.WhiteSmallText().WithFontSize(12).WithWeight(Cairo.FontWeight.Bold),
            ...
            else
            {
                ...
                var abandonButton = new GuiElementTextButton(capi, Lang.Get("soaguildsandkingdoms:quests-abandon"),
                    CairoFont.WhiteSmallText().WithFontSize(12).WithWeight(Cairo.FontWeight.Bold),
                    CairoFont.WhiteSmallText().WithFontSize(12).WithWeight(Cairo.FontWeight.Bold),
        ...
        private double RenderAvailableQuestAsElement(GuiElementContainer container, double yPos, int magicWidth, QuestDto quest)
        {
            ...
            var buttonText = Lang.Get("soaguildsandkingdoms:quests-start");
            var buttonWidth = 80.0;

            if (needsMonthlyBoard)
            {
                buttonText = Lang.Get("soaguildsandkingdoms:quests-needs-monthly-board");
                buttonWidth = 165.0;
            }

            if (needsDailyBoard)
            {
                buttonText = Lang.Get("soaguildsandkingdoms:quests-needs-daily-board");
                buttonWidth = 165.0;
            }

            if (alreadyCompletedLastWeek)
            {
                buttonText = Lang.Get("soaguildsandkingdoms:quests-recently-completed");
                buttonWidth = 150.0;
            }

            if (isPeriodActive && quest.RecurrenceType != "weekly")
            {
                buttonText = Lang.Get("soaguildsandkingdoms:quests-period-in-progress");
                buttonWidth = 120.0;
            }
        ...
        private string GetNextResetTimeDisplay(string expiresAt)
        {
            try
            {
                ...
                return Lang.Get("soaguildsandkingdoms:quests-next-reset", $"{resetTimeLocal:MMMM d, yyyy h:mm tt} {tzAbbreviation}");
            }
            catch
            {
                return "";
        ...
        private void OnAbandonQuestClicked(int questId)
        {
            ...
            var confirmDialog = new ConfirmAbandonDialog(
                capi,
                Lang.Get("soaguildsandkingdoms:quests-abandon-confirm-message", questTitle),
        ...
        private void OnStartResponse(QuestStartResponsePacket response)
        {
            if (response.Success)
            {
                capi.ShowChatMessage(Lang.Get("soaguildsandkingdoms:quests-started-success"));
                // Refresh quest data to update UI
                FetchQuestData();
            }
            else
            {
                capi.ShowChatMessage(Lang.Get("soaguildsandkingdoms:quests-started-failed", response.Message));
            }
        }
        ...
        private void OnAbandonResponse(QuestAbandonResponsePacket response)
        {
            if (response.Success)
            {
                capi.ShowChatMessage(Lang.Get("soaguildsandkingdoms:quests-abandoned"));
                FetchQuestData();
            }
            else
            {
                capi.ShowChatMessage(Lang.Get("soaguildsandkingdoms:quests-abandon-failed", response.Message));
            }
        }
        ...
        private void OnSubmitPreviewReceived(QuestSubmitPreviewResponsePacket response)
        {
            if (!response.Success)
            {
                capi.ShowChatMessage(Lang.Get("soaguildsandkingdoms:quests-submit-failed", response.Message));
                return;
            }
            ...
            var confirmDialog = new ConfirmSubmitDialog(
                capi,
                Lang.Get("soaguildsandkingdoms:quests-submit-confirm-message"),
                lines,
        ...
        private void OnSubmitConfirmReceived(QuestSubmitConfirmResponsePacket response)
        {
            if (response.Success)
            {
                capi.ShowChatMessage(Lang.Get("soaguildsandkingdoms:quests-submit-success"));
                FetchQuestData();
            }
            else
            {
        ...
        private void OnCompleteReceived(QuestCompleteResponsePacket response)
        {
            if (response.Success)
            {
                capi.ShowChatMessage(Lang.Get("soaguildsandkingdoms:quests-complete-success"));
            ...
            {
                capi.ShowChatMessage(Lang.Get("soaguildsandkingdoms:quests-complete-failed", response.Message));
            }
        }
    ...
    public class ConfirmAbandonDialog : GuiDialog
    {
        ...
        private void SetupDialog()
        {
            ...
            SingleComposer = capi.Gui.CreateCompo("guild-quest-abandon-confirm", dialogBounds)
                ...
                .AddDialogTitleBar(Lang.Get("soaguildsandkingdoms:quests-abandon-confirm-title"), OnTitleBarClose)
            ...
            SingleComposer.AddSmallButton(Lang.Get("soaguildsandkingdoms:quests-abandon-confirm-yes"), OnConfirmClick, confirmBounds);
            SingleComposer.AddSmallButton(Lang.Get("soaguildsandkingdoms:quests-abandon-confirm-no"), OnCancelClick, cancelBounds);
    ...
    public class ConfirmSubmitDialog : GuiDialog
    {
        ...
        private void SetupDialog()
        {
            ...
            SingleComposer = capi.Gui.CreateCompo("guild-quest-submit-confirm", dialogBounds)
                .AddShadedDialogBG(bgBounds)
                .AddDialogTitleBar(Lang.Get("soaguildsandkingdoms:quests-submit-confirm-title"), OnTitleBarClose)
                .BeginChildElements(bgBounds);
            ...
            SingleComposer.AddSmallButton(Lang.Get("soaguildsandkingdoms:quests-submit-confirm-yes"), OnConfirmClick, confirmBounds);
            SingleComposer.AddSmallButton(Lang.Get("soaguildsandkingdoms:quests-submit-confirm-no"), OnCancelClick, cancelBounds);
