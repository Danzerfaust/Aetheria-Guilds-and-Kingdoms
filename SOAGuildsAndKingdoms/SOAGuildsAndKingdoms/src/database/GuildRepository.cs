...
namespace SOAGuildsAndKingdoms.src.database
{
    ...
    public class GuildRepository(ICoreServerAPI serverApi, GuildDatabase database)
    {
        ...
        public void LoadAllGuilds()
        {
            try
            {
                serverApi.Logger.Notification("[SOAGuildsAndKingdoms] Loading guilds from db...");
                ...
                cacheLoaded = true;
                serverApi.Logger.Notification($"[SOAGuildsAndKingdoms] Loaded {guildCache.Count} guilds from db");
            }
            catch (Exception ex)
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms] Failed to load guilds: {ex.Message}");
        ...
        public void CommitChanges()
        {
            if (!cacheLoaded)
            {
                serverApi.Logger.Warning("[SOAGuildsAndKingdoms] Cannot save guild changes: cache not loaded");
                return;
            }
            ...
            try
            {
                ...
                try
                {
                    ...
                    serverApi.Logger.Notification($"[SOAGuildsAndKingdoms] Saved {savedCount} guild changes to db");
                }
                catch (Exception ex)
                {
                    transaction.Rollback();
                    serverApi.Logger.Error($"[SOAGuildsAndKingdoms] Transaction rolled back due to error: {ex.Message}");
                    throw;
                }
            }
            catch (Exception ex)
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms] Failed to commit changes: {ex.Message}");
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms] Stack trace: {ex.StackTrace}");
                throw;
            }
        }
        ...
        public List<Guild> GetLeaderboard(int limit = 10)
        {
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms] Failed to get leaderboard: {ex.Message}");
                return [];
            }
        }
