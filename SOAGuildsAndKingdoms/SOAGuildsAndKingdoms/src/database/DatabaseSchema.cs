    ...
    public class DatabaseSchema
    {
        ...
        public void InitializeSchema(SqliteConnection connection)
        {
            try
            {
                ...
                if (currentVersion == 0)
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms] Initializing database schema...");
                    CreateAllTables(connection);
                    SetSchemaVersion(connection, CurrentSchemaVersion);
                    serverApi.Logger.Notification($"[SOAGuildsAndKingdoms] Schema initialized to version {CurrentSchemaVersion}");
                }
                else if (currentVersion < CurrentSchemaVersion)
                {
                    serverApi.Logger.Notification($"[SOAGuildsAndKingdoms] Migrating schema from version {currentVersion} to {CurrentSchemaVersion}");
                    ApplyMigrations(connection, currentVersion);
                    SetSchemaVersion(connection, CurrentSchemaVersion);
                    serverApi.Logger.Notification($"[SOAGuildsAndKingdoms] Schema migration complete");
                }
                else if (currentVersion > CurrentSchemaVersion)
                {
                    serverApi.Logger.Error($"[SOAGuildsAndKingdoms] Database schema version {currentVersion} is newer than expected {CurrentSchemaVersion}. Please update the mod.");
                    throw new InvalidOperationException($"Database schema version mismatch: {currentVersion} > {CurrentSchemaVersion}");
                }
            }
            catch (Exception ex)
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms] Failed to initialize schema: {ex.Message}");
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms] Stack trace: {ex.StackTrace}");
                throw;
            }
        }
        ...
        private void CreateAllTables(SqliteConnection connection)
        {
            ...
            try
            {
                ...
                transaction.Commit();
                serverApi.Logger.Notification("[SOAGuildsAndKingdoms] All tables created successfully");
            }
            catch (Exception ex)
            {
                transaction.Rollback();
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms] Failed to create tables: {ex.Message}");
                throw;
            }
        }
        ...
        private void ApplyMigrations(SqliteConnection connection, int fromVersion)
        {
            ...
            try
            {
                if (fromVersion < 2)
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms] Applying migration to version 2: Adding repeat column to guild_quests");
                    ExecuteNonQuery(connection, "ALTER TABLE guild_quests ADD COLUMN repeat INTEGER NOT NULL DEFAULT 0;", transaction);
                }

                if (fromVersion < 3)
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms] Applying migration to version 3: Adding points_contribution column to guild_members and removing composite PRIMARY KEY from guild_member_quests");
                    ...
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms] Calculating points_contribution for existing members...");
                    ExecuteNonQuery(connection, @"
                ...
                if (fromVersion < 4)
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms] Applying migration to version 4: Adding nodes table");
                ...
                if (fromVersion < 5)
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms] Applying migration to version 5: Adding capture_zones table");
                ...
                if (fromVersion < 6)
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms] Applying migration to version 6: Adding node_wars tables");
                ...
                if (fromVersion < 7)
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms] Applying migration to version 7: Adding rank column to guild_quests, removing daily/weekly quests, and removing period lock index");

                    ExecuteNonQuery(connection, "DELETE FROM guild_quests WHERE recurrence_type IN ('daily', 'weekly');", transaction);
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms] Removed all daily and weekly quests from guild_quests table");
                    ...
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms] Removed idx_period_lock unique index");
                }

                if (fromVersion < 8)
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms] Applying migration to version 8: Adding guild_weekly_points table for weekly GRS points tracking");
                ...
                if (fromVersion < 9)
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms] Applying migration to version 9: Adding guild_events and guild_event_registrations tables");
            ...
            {
                transaction.Rollback();
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms] Failed to apply migrations: {ex.Message}");
                throw;
            }
        }
        ...
        public bool VerifySchema(SqliteConnection connection)
        {
            try
            {
                ...
                foreach (var tableName in requiredTables)
                {
                    ...
                    {
                        serverApi.Logger.Error($"[SOAGuildsAndKingdoms] Required table '{tableName}' does not exist");
                        return false;
                    }
                }
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms] Schema verification failed: {ex.Message}");
                return false;
            }
        }
