    ...
    public class ZoneWhitelistRepository
    {
        ...
        public bool AddPlayerToZone(int zoneId, string playerUid)
        {
            ...
            try
            {
                ...
                if (rowsAffected > 0)
                {
                    serverApi.Logger.Debug($"[SOAGuildsAndKingdoms:ZoneWhitelistRepository] Added player '{playerUid}' to zone ID {zoneId}");
                }
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:ZoneWhitelistRepository] Failed to add player to zone: {ex.Message}");
                return false;
            }
        }
        ...
        public bool RemovePlayerFromZone(int zoneId, string playerUid)
        {
            ...
            try
            {
                ...
                if (rowsAffected > 0)
                {
                    serverApi.Logger.Debug($"[SOAGuildsAndKingdoms:ZoneWhitelistRepository] Removed player '{playerUid}' from zone ID {zoneId}");
                }
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:ZoneWhitelistRepository] Failed to remove player from zone: {ex.Message}");
                return false;
            }
        }
        ...
        public bool IsPlayerWhitelisted(int zoneId, string playerUid)
        {
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:ZoneWhitelistRepository] Failed to check whitelist: {ex.Message}");
                return false;
            }
        }
        ...
        public List<int> GetWhitelistedZones(string playerUid)
        {
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:ZoneWhitelistRepository] Failed to get whitelisted zones: {ex.Message}");
            }
        ...
        public List<string> GetWhitelistedPlayers(int zoneId)
        {
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:ZoneWhitelistRepository] Failed to get whitelisted players: {ex.Message}");
            }
        ...
        public int ClearZone(int zoneId)
        {
            ...
            try
            {
                ...
                if (rowsDeleted > 0)
                {
                    serverApi.Logger.Debug($"[SOAGuildsAndKingdoms:ZoneWhitelistRepository] Cleared {rowsDeleted} player(s) from zone ID {zoneId}");
                }
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:ZoneWhitelistRepository] Failed to clear zone: {ex.Message}");
                return 0;
            }
        }
        ...
        public List<int> GetAllZoneIds()
        {
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:ZoneWhitelistRepository] Failed to get zone IDs: {ex.Message}");
            }
        ...
        public int GetTotalWhitelistedPlayersCount()
        {
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:ZoneWhitelistRepository] Failed to get total player count: {ex.Message}");
                return 0;
            }
        }
        ...
        public Dictionary<int, List<string>> GetAllWhitelists()
        {
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:ZoneWhitelistRepository] Failed to get all whitelists: {ex.Message}");
            }
