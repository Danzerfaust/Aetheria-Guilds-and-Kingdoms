    ...
    public class NodeRepository
    {
        ...
        public int AddNode(string name, int x, int z, int radius)
        {
            ...
            try
            {
                ...
                if (nodeId > 0)
                {
                    serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:NodeRepository] Created node '{name}' at ({x}, {z}) with radius {radius} (ID: {nodeId})");
                }
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to add node: {ex.Message}");
                return -1;
            }
        }
        ...
        public bool RemoveNode(int nodeId)
        {
            ...
            try
            {
                ...
                if (rowsAffected > 0)
                {
                    serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:NodeRepository] Deleted node ID {nodeId}");
                }
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to remove node: {ex.Message}");
                return false;
            }
        }
        ...
        public bool RemoveNodeByName(string name)
        {
            ...
            try
            {
                ...
                if (rowsAffected > 0)
                {
                    serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:NodeRepository] Deleted node '{name}'");
                }
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to remove node by name: {ex.Message}");
                return false;
            }
        }
        ...
        public bool UpdateNode(int nodeId, string? name = null, int? x = null, int? z = null, int? radius = null)
        {
            ...
            try
            {
                ...
                if (rowsAffected > 0)
                {
                    serverApi.Logger.Debug($"[SOAGuildsAndKingdoms:NodeRepository] Updated node ID {nodeId}");
                }
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to update node: {ex.Message}");
                return false;
            }
        }
        ...
        public NodeData? GetNode(int nodeId)
        {
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to get node: {ex.Message}");
            }
        ...
        public NodeData? GetNodeByName(string name)
        {
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to get node by name: {ex.Message}");
            }
        ...
        public List<NodeData> GetAllNodes()
        {
            ...
            try
            {
                ...
                serverApi.Logger.Debug($"[SOAGuildsAndKingdoms:NodeRepository] Loaded {nodes.Count} nodes from database");
            }
            catch (Exception ex)
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to get all nodes: {ex.Message}");
            }
        ...
        public bool NodeExists(string name)
        {
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to check node existence: {ex.Message}");
                return false;
            }
        }
        ...
        public int GetNodeCount()
        {
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to get node count: {ex.Message}");
                return 0;
            }
        }
            ...
            int radius, double pointMultiplier = 1.0, bool isActive = true, string? description = null)
        {
            ...
            try
            {
                ...
                if (captureZoneId > 0)
                {
                    serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:NodeRepository] Created capture zone '{zoneName}' ({zoneId}) for node '{nodeName}' (ID: {captureZoneId})");
                }
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to add capture zone: {ex.Message}");
                return -1;
            }
        }
        ...
        public bool RemoveCaptureZone(string nodeName, string zoneId)
        {
            ...
            try
            {
                ...
                if (rowsAffected > 0)
                {
                    serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:NodeRepository] Deleted capture zone '{zoneId}' from node '{nodeName}'");
                }
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to remove capture zone: {ex.Message}");
                return false;
            }
        }
        ...
        public List<CaptureZoneData> GetCaptureZonesForNode(string nodeName)
        {
            ...
            try
            {
                ...
                serverApi.Logger.Debug($"[SOAGuildsAndKingdoms:NodeRepository] Loaded {zones.Count} capture zones for node '{nodeName}'");
            }
            catch (Exception ex)
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to get capture zones: {ex.Message}");
            }
        ...
        public List<CaptureZoneData> GetAllCaptureZones()
        {
            ...
            try
            {
                ...
                serverApi.Logger.Debug($"[SOAGuildsAndKingdoms:NodeRepository] Loaded {zones.Count} total capture zones");
            }
            catch (Exception ex)
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to get all capture zones: {ex.Message}");
            }
            ...
            double? centerY = null, double? centerZ = null, int? radius = null, double? pointMultiplier = null, bool? isActive = null, string? description = null)
        {
            ...
            try
            {
                ...
                if (rowsAffected > 0)
                {
                    serverApi.Logger.Debug($"[SOAGuildsAndKingdoms:NodeRepository] Updated capture zone '{zoneId}' in node '{nodeName}'");
                }
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to update capture zone: {ex.Message}");
                return false;
            }
        }
        ...
        public bool RemoveAllCaptureZonesForNode(string nodeName)
        {
            ...
            try
            {
                ...
                if (rowsAffected > 0)
                {
                    serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:NodeRepository] Deleted {rowsAffected} capture zones from node '{nodeName}'");
                }
            ...
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:NodeRepository] Failed to remove all capture zones: {ex.Message}");
                return false;
            }
        }
