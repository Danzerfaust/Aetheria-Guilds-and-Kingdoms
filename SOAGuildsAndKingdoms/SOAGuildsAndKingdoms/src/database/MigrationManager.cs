...
namespace SOAGuildsAndKingdoms.src.database
{
    ...
    public class MigrationManager
    {
        ...
        public MigrationManager(ICoreServerAPI serverApi, GuildDatabase database)
        {
            ...
            this.dataFolder = serverApi.GetOrCreateDataPath("ModData/SOAGuildsAndKingdoms");
        ...
        public bool NeedsMigration()
        {
            try
            {
                ...
                {
                    serverApi.Logger.Warning("[SOAGuildsAndKingdoms:MigrationManager] Both JSON and DB exist - delete guilds.db if you wanna remigrate, otherwise delete jsons");
                    return false;
                }

                serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Migration needed for: guilds={hasGuildsJson}, cooldowns={hasCooldownJson}, whitelists={hasWhitelistJson}");
                return hasGuildsJson || hasCooldownJson || hasWhitelistJson;
            }
            catch (Exception ex)
            {
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:MigrationManager] Error checking migration status: {ex.Message}");
                return false;
            }
        }
        ...
        public MigrationResult MigrateFromJson()
        {
            ...
            try
            {
                serverApi.Logger.Notification("[SOAGuildsAndKingdoms:MigrationManager] ========================================");
                serverApi.Logger.Notification("[SOAGuildsAndKingdoms:MigrationManager] Starting migration from JSON to SQLite");
                serverApi.Logger.Notification("[SOAGuildsAndKingdoms:MigrationManager] ========================================");

                // Step 1: Create backups
                serverApi.Logger.Notification("[SOAGuildsAndKingdoms:MigrationManager] Step 1: Creating backups of JSON files...");
                CreateBackups(result);

                // Step 2: Load and migrate guilds
                if (File.Exists(guildsJsonPath))
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms:MigrationManager] Step 2: Migrating guild data...");
                    MigrateGuilds(result);
                }
                else
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms:MigrationManager] Step 2: Skipped - no guilds.json found");
                }

                // Step 3: Migrate cooldowns
                if (File.Exists(cooldownJsonPath))
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms:MigrationManager] Step 3: Migrating cooldown data...");
                    MigrateCooldowns(result);
                }
                else
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms:MigrationManager] Step 3: Skipped - no guild_cooldowns.json found");
                }

                // Step 4: Migrate zone whitelists
                if (File.Exists(zoneWhitelistJsonPath))
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms:MigrationManager] Step 4: Migrating zone whitelist data...");
                    MigrateZoneWhitelists(result);
                }
                else
                {
                    serverApi.Logger.Notification("[SOAGuildsAndKingdoms:MigrationManager] Step 4: Skipped - no zone_whitelist.json found");
                }

                // Step 5: Validate migration
                serverApi.Logger.Notification("[SOAGuildsAndKingdoms:MigrationManager] Step 5: Validating migration...");
                ...
                serverApi.Logger.Notification("[SOAGuildsAndKingdoms:MigrationManager] ========================================");
                serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Migration completed in {result.Duration.TotalSeconds:F2} seconds");
                serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Success: {result.Success}");
                serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Guilds migrated: {result.GuildsMigrated}");
                serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Cooldowns migrated: {result.CooldownsMigrated}");
                serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Zone whitelists migrated: {result.ZoneWhitelistsMigrated}");

                if (result.Errors.Any())
                {
                    serverApi.Logger.Error($"[SOAGuildsAndKingdoms:MigrationManager] Errors encountered: {result.Errors.Count}");
                    foreach (var error in result.Errors)
                    {
                        serverApi.Logger.Error($"[SOAGuildsAndKingdoms:MigrationManager]   - {error}");
                    }
                }

                if (result.Warnings.Any())
                {
                    serverApi.Logger.Warning($"[SOAGuildsAndKingdoms:MigrationManager] Warnings: {result.Warnings.Count}");
                    foreach (var warning in result.Warnings)
                    {
                        serverApi.Logger.Warning($"[SOAGuildsAndKingdoms:MigrationManager]   - {warning}");
                    }
                }
            ...
            catch (Exception ex)
            {
                ...
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:MigrationManager] Migration failed: {ex.Message}");
                serverApi.Logger.Error($"[SOAGuildsAndKingdoms:MigrationManager] Stack trace: {ex.StackTrace}");
        ...
        private void CreateBackups(MigrationResult result)
        {
            ...
            if (File.Exists(guildsJsonPath))
            {
                ...
                serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Created backup: {backupPath}");
            }
        ...
        private void MigrateGuilds(MigrationResult result)
        {
            try
            {
                ...
                serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Loaded {guilds.Count} guilds from JSON");
                ...
                try
                {
                    foreach (var guildKvp in guilds)
                    {
                        ...
                        try
                        {
                            ...
                            serverApi.Logger.Debug($"[SOAGuildsAndKingdoms:MigrationManager] Migrated guild: {guild.Name} ({guild.Members.Count} members, {guild.Claims.Count} claims)");
                        }
                        catch (Exception ex)
                        {
                            result.Errors.Add($"Failed to migrate guild '{guild.Name}': {ex.Message}");
                            serverApi.Logger.Error($"[SOAGuildsAndKingdoms:MigrationManager] Error migrating guild '{guild.Name}': {ex.Message}");
                        }
                    }

                    transaction.Commit();
                    serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Successfully migrated {result.GuildsMigrated} guilds");
                }
                catch (Exception ex)
                {
        ...
        private void MigrateCooldowns(MigrationResult result)
        {
            try
            {
                ...
                try
                {
                    ...
                    transaction.Commit();
                    serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Successfully migrated {result.CooldownsMigrated} cooldowns (expired cooldowns skipped)");
                }
                catch (Exception ex)
                {
                    transaction.Rollback();
            ...
            try
            {
                ...
                try
                {
                    ...
                    foreach (var whitelistKvp in whitelists.OrderBy(kvp => kvp.Key))
                    {
                        ...
                        serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Migrated zone '{zoneName}' -> ID {zoneId} with {playerUids.Count} player(s)");
                        zoneId++;
                    }

                    transaction.Commit();
                    serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Successfully migrated {result.ZoneWhitelistsMigrated} zone whitelist entries");
        ...
        private void ValidateMigration(MigrationResult result)
        {
            try
            {
                ...
                using (var command = new Microsoft.Data.Sqlite.SqliteCommand(countGuildsSql, connection))
                {
                    ...
                    {
                        serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Validation: Guild count matches ({dbGuildCount})");
                    }
                }
                ...
                using (var command = new Microsoft.Data.Sqlite.SqliteCommand(countCooldownsSql, connection))
                {
                    ...
                    {
                        serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Validation: Cooldown count matches ({dbCooldownCount})");
                    }
                }
                ...
                using (var command = new Microsoft.Data.Sqlite.SqliteCommand(countWhitelistsSql, connection))
                {
                    ...
                    {
                        serverApi.Logger.Notification($"[SOAGuildsAndKingdoms:MigrationManager] Validation: Zone whitelist count matches ({dbWhitelistCount})");
                    }
                }

                serverApi.Logger.Notification("[SOAGuildsAndKingdoms:MigrationManager] Validation complete");
            }
            catch (Exception ex)
            {
                result.Warnings.Add($"Validation failed: {ex.Message}");
                serverApi.Logger.Warning($"[SOAGuildsAndKingdoms:MigrationManager] Validation encountered errors: {ex.Message}");
            }
        }
