...
using SOAGuildsAndKingdoms.src.gui;
using SOAGuildsAndKingdoms.src.guilds;
using SOAGuildsAndKingdoms.src.guilds.behaviors;
...
using SOAGuildsAndKingdoms.src.quests.blocks;
...
using GuildLandClaim = SOAGuildsAndKingdoms.src.guilds.LandClaim;

namespace SOAGuildsAndKingdoms
{
    public class SOAGuildsAndKingdomsModSystem : ModSystem
    {
        ...
        public override void Start(ICoreAPI api)
        {
            ...
            // Initialize Harmony
            harmony = new Harmony("soaguildsandkingdoms.patches");
            ...
            try
            {
                ...
                {
                    var prefixMethod = typeof(SOAGuildsAndKingdoms.src.patches.WorldMapTestBlockAccessPatch).GetMethod("Prefix");
                ...
                try
                {
                    ...
                    if (largeDoorBehaviorType != null)
                    {
                        ...
                        {
                            var prefixMethod = typeof(SOAGuildsAndKingdoms.src.patches.PortcullisLargeDoorPatch).GetMethod("PrefixController");
                            var postfixMethod = typeof(SOAGuildsAndKingdoms.src.patches.PortcullisLargeDoorPatch).GetMethod("PostfixController");
                ...
                try
                {
                    ...
                    if (cellBlockType != null)
                    {
                        ...
                        {
                            var cellPrefixMethod = typeof(SOAGuildsAndKingdoms.src.patches.PortcullisLargeDoorPatch).GetMethod("PrefixCell");
                            var cellPostfixMethod = typeof(SOAGuildsAndKingdoms.src.patches.PortcullisLargeDoorPatch).GetMethod("PostfixCell");
        ...
        {
            harmony?.UnpatchAll("soaguildsandkingdoms.patches");
            base.Dispose();
        }
        ...
        public override void StartServerSide(ICoreServerAPI api)
        {
            ...
            try
            {
                ...
                {
                    api.Logger.Warning("[SOAGuildsAndKingdoms:Migration] JSON guild data detected - running migration to SQLite...");
                    var result = migrationManager.MigrateFromJson();

                    if (result.Success)
                    {
                        api.Logger.Notification($"[SOAGuildsAndKingdoms:Migration] Migration completed successfully:");
                        ...
                        {
                            api.Logger.Warning($"[SOAGuildsAndKingdoms:Migration] Migration had {result.Warnings.Count} warnings:");
                        ...
                        api.Logger.Notification("[SOAGuildsAndKingdoms:Migration] Reloading guilds from database after migration...");
                        guildRepository.LoadAllGuilds();
                        var loadedCount = guildRepository.GetAllGuilds().Count;
                        api.Logger.Notification($"[SOAGuildsAndKingdoms:Migration] Loaded {loadedCount} guild(s) into cache");
                    }
                    else
                    {
                        api.Logger.Error($"[SOAGuildsAndKingdoms:Migration] Migration FAILED!");
                ...
                api.Logger.Notification("[SOAGuildsAndKingdoms] Database initialization complete");
            }
            catch (Exception ex)
            {
                api.Logger.Error($"[SOAGuildsAndKingdoms] Failed to initialize database: {ex.Message}");
            ...
            api.Logger.Notification($"[SOAGuildsAndKingdoms] Land claim spatial indexes built: {totalClaims} chunks across {guildsWithClaims} guilds");
            ...
            api.Event.PlayerJoin += (player) =>
            {
                ...
                api.Event.RegisterCallback((dt) =>
                {
                    ...
                    {
                        Mod.Logger.Notification($"[SOAGuildsAndKingdoms:GuildSync] Syncing traits and guild summaries for player {serverPlayer.PlayerName} on join");
            ...
            api.Event.RegisterGameTickListener((dt) =>
            {
                if (guildManager != null && serverApi != null)
                {
                    ...
                    {
                        Mod.Logger.Debug($"[SOAGuildsAndKingdoms:TraitSync] Running periodic trait sync for {onlinePlayers.Length} online players");
            ...
            api.ChatCommands.Create("zonewhitelist")
                ...
                .WithAlias("zonewl")
                .RequiresPrivilege("soaguildsandkingdoms:zonewhitelist")
                .BeginSubCommand("add")
            ...
            api.ChatCommands.Create("questmanager")
                ...
                .WithAlias("qm")
                .RequiresPrivilege("soaguildsandkingdoms:questmanager")
            ...
            api.ChatCommands.Create("eventmanager")
                ...
                .WithAlias("em")
                .RequiresPrivilege("soaguildsandkingdoms:eventmanager")
        ...
        private TextCommandResult HandleListInvites(IServerPlayer player)
        {
            ...
            serverApi!.Network.GetChannel("soaguildsandkingdoms:guild")!.SendPacket(response, player);
        ...
        private TextCommandResult OnQuestManagerCommand(TextCommandCallingArgs args)
        {
            ...
            serverApi!.Network.GetChannel("soaguildsandkingdoms:quest")!.SendPacket(packet, player);
        ...
        private TextCommandResult OnDebugInviteCommand(TextCommandCallingArgs args)
        {
            ...
            serverApi!.Network.GetChannel("soaguildsandkingdoms:guild")!
        ...
        private void ProcessExpiredRepeatingQuests()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (cleanedCount > 0)
                    {
                        serverApi?.Logger.Notification($"[SOAGuildsAndKingdoms:Quests] Cleaned up {cleanedCount} stale active quest progress entries");
                    }
                }
                catch (Exception ex)
                {
                    serverApi?.Logger.Error($"[SOAGuildsAndKingdoms:Quests] Failed to cleanup stale quest progress: {ex.Message}");
                }
                ...
                serverApi?.Logger.Notification($"[SOAGuildsAndKingdoms:Quests] Renewing {expiredQuests.Count} expired repeating quest(s)");
                ...
                foreach (var quest in expiredQuests)
                {
                    try
                    {
                        ...
                        if (!updated)
                        {
                            serverApi?.Logger.Error($"[SOAGuildsAndKingdoms:Quests] Failed to renew quest {quest.Id} '{quest.Title}' in database");
                        }
                    }
                    catch (Exception ex)
                    {
                        serverApi?.Logger.Error($"[SOAGuildsAndKingdoms:Quests] Failed to renew quest {quest.Id} '{quest.Title}': {ex.Message}");
                    }
                }
            }
            catch (Exception ex)
            {
                serverApi?.Logger.Error($"[SOAGuildsAndKingdoms:Quests] Failed to process expired repeating quests: {ex.Message}");
            }
        }
        ...
        private void ProcessWeeklyGrsReset()
        {
            ...
            try
            {
                ...
                if (cleaned > 0)
                {
                    serverApi?.Logger.Debug($"[SOAGuildsAndKingdoms:WeeklyGRS] Cleaned up {cleaned} old weekly point entries (before week {cutoffWeekKey})");
                }
            }
            catch (Exception ex)
            {
                serverApi?.Logger.Error($"[SOAGuildsAndKingdoms:WeeklyGRS] Failed to process weekly GRS reset: {ex.Message}");
            }
        }
        ...
        private void OnSaveGameSaving()
        {
            ...
            try
            {
                ...
                serverApi?.Logger.Debug("[SOAGuildsAndKingdoms:Database] Database checkpoint complete");
            }
            catch (Exception ex)
            {
                serverApi?.Logger.Error($"[SOAGuildsAndKingdoms:Database] Database checkpoint failed: {ex.Message}");
            }
        ...
        private void CreateGuildBackup()
        {
            try
            {
                ...
                var backupDir = System.IO.Path.Combine(
                    serverApi!.GetOrCreateDataPath("ModData/SOAGuildsAndKingdoms"),
                    "backups"
                );
                ...
                if (System.IO.File.Exists(dbPath))
                {
                    ...
                    Mod.Logger.Debug($"[SOAGuildsAndKingdoms:Backup] Database backed up to: {dbBackupPath}");
                    ...
                    if (System.IO.File.Exists(walPath))
                    {
                        ...
                        Mod.Logger.Debug($"[SOAGuildsAndKingdoms:Backup] WAL file backed up to: {walBackupPath}");
                    }
                    ...
                    if (System.IO.File.Exists(shmPath))
                    {
                        ...
                        Mod.Logger.Debug($"[SOAGuildsAndKingdoms:Backup] SHM file backed up to: {shmBackupPath}");
                    }
                }
            ...
            {
                Mod.Logger.Error($"[SOAGuildsAndKingdoms:Backup] Failed to create database backup: {ex.Message}");
            }
        }
        ...
        private void CleanupOldBackups(string backupDir, int maxBackupsToKeep)
        {
            ...
            {
                Mod.Logger.Warning($"[SOAGuildsAndKingdoms:Backup] Failed to cleanup old backups: {ex.Message}");
            }
        }
        ...
        private void ProcessBackupType(string backupDir, string searchPattern, string backupType, int maxBackupsToKeep)
        {
            try
            {
                ...
                Mod.Logger.Debug($"[SOAGuildsAndKingdoms:Backup] Processing {oldBackups.Count} old {backupType} backups across {backupsByDate.Count} days");
                ...
                foreach (var dailyGroup in backupsByDate)
                {
                        ...
                        using (var archive = ZipFile.Open(zipFileName, ZipArchiveMode.Update))
                        {
                            foreach (var file in dailyGroup)
                            {
                                ...
                                if (existingEntry == null)
                                {
                                    ...
                                    Mod.Logger.Debug($"[SOAGuildsAndKingdoms:Backup] Added {entryName} to existing archive {dateString}");
                                }
                    ...
                    else
                    {
                        ...
                        Mod.Logger.Debug($"[SOAGuildsAndKingdoms:Backup] Created {backupType} archive for {dateString} with {dailyGroup.Count()} backup(s)");
                    }
                }
            ...
            {
                Mod.Logger.Warning($"[SOAGuildsAndKingdoms:Backup] Failed to process {backupType} backups: {ex.Message}");
            }
        }
        ...
        private void CleanupOldZipArchives(string backupDir, string backupType, int daysToKeep)
        {
            try
            {
                ...
                foreach (var archive in oldArchives)
                {
                    ...
                    Mod.Logger.Debug($"[SOAGuildsAndKingdoms:Backup] Deleted old archive: {System.IO.Path.GetFileName(archive)}");
                }

                if (oldArchives.Count > 0)
                {
                    Mod.Logger.Notification($"[SOAGuildsAndKingdoms:Backup] Deleted {oldArchives.Count} {backupType} archive(s) older than {daysToKeep} days");
                }
            }
            catch (Exception ex)
            {
                Mod.Logger.Warning($"[SOAGuildsAndKingdoms:Backup] Failed to cleanup old {backupType} archives: {ex.Message}");
            }
        }
        ...
        public void ToggleHologram()
        {
            ...
            {
                ShowClaimsHologram();
                clientApi?.ShowChatMessage(Lang.Get("soaguildsandkingdoms:hologram-shown"));
            }
            else
            {
                ClearHologram();
                clientApi?.ShowChatMessage(Lang.Get("soaguildsandkingdoms:hologram-hidden"));
            }
        }
        ...
        public void ShowClaimsHologram()
        {
            ...
            if (currentGuild == null || currentGuild.Claims.Count == 0)
            {
                ...
                if (hologramVisible)
                {
                    ...
                    clientApi.ShowChatMessage(Lang.Get("soaguildsandkingdoms:hologram-no-claims"));
                }
                return;
            }
        ...
        private void OnGuildSummariesReceived(List<GuildSummary> summaries)
        {
            ...
            if (clientApi != null)
            {
                ...
                if (playerGuild != null)
                {
                    Mod.Logger.Notification($"[SOAGuildsAndKingdoms:GuildSync] Received guild summaries - Player is member of: {playerGuild.Name}");
                }
                else
                {
                    Mod.Logger.Debug($"[SOAGuildsAndKingdoms:GuildSync] Received {summaries.Count} guild summaries - Player is not in any guild");
                }
            }
