...
namespace SRGuildsAndKingdoms.src.gui
{
    ...
    public class DialogQuestEditor : GuiDialog
    {
        ...
        public DialogQuestEditor(ICoreClientAPI capi, SRGuildsAndKingdomsModSystem modSystem, QuestDto? quest = null, CurrencyDefinitionDto? tailsDefinition = null, CurrencyDefinitionDto? crownsDefinition = null, long serverLocalTime = 0, double serverTimezoneOffset = 0)
            : base(capi)
        {
            ...
            objectiveInventory = new InventoryGeneric(1000, "srguildsandkingdoms:questeditor-objectives", capi, (id, inv) =>
            {
                return new ItemSlotDisplayOnly(inv) { MaxSlotStackSize = 1 };
            });
            objectiveInventory.SlotModified += OnInventorySlotModified;

            rewardsInventory = new InventoryGeneric(10, "srguildsandkingdoms:questeditor-rewards", capi, (id, inv) =>
            {
                return new ItemSlotDisplayOnly(inv) { MaxSlotStackSize = 1 };
            });
