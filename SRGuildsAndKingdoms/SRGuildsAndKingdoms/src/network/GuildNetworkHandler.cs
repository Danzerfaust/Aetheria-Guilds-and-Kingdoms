...
namespace SRGuildsAndKingdoms.src.network
{
    ...
    public class GuildNetworkHandler
    {
        ...
        public void InitializeServer(ICoreServerAPI api, GuildManager manager)
        {
            ...
            modSystem = api.ModLoader.GetModSystem<SRGuildsAndKingdomsModSystem>();
        ...
        private void UpdateMemberLastSeen(string playerUid)
        {
            ...
            if (guild != null && guild.Members.ContainsKey(playerUid))
            {
                ...
                var modSystem = serverApi?.ModLoader.GetModSystem<SRGuildsAndKingdomsModSystem>();
            ...
            switch (packet.Command?.ToLowerInvariant())
            {
                ...
                case "changecolor":
                    if (packet.Arguments?.Length > 0 && int.TryParse(packet.Arguments[0], out int primaryColor))
                    {
                        ...
                        var guildRepo = serverApi?.ModLoader.GetModSystem<SRGuildsAndKingdomsModSystem>()?.GetGuildRepository();
        ...
        private void OnGuildClaimLandReceived(IServerPlayer player, GuildClaimLandPacket packet)
        {
            ...
            int chunkX = SRGuildsAndKingdoms.src.guilds.LandClaim.FloorDiv(packet.BlockX, SRGuildsAndKingdoms.src.guilds.LandClaim.ChunkSize);
            int chunkZ = SRGuildsAndKingdoms.src.guilds.LandClaim.FloorDiv(packet.BlockZ, SRGuildsAndKingdoms.src.guilds.LandClaim.ChunkSize);
        ...
        private void OnGuildInviteNotificationReceived(GuildInviteNotificationPacket packet)
        {
            ...
            var modSystem = clientApi?.ModLoader.GetModSystem<SRGuildsAndKingdomsModSystem>();
        ...
        public void SendGuildClaimLandRequest(int chunkX, int chunkZ, bool isOutpost, string outpostName = "")
        {
            ...
            int blockX = chunkX * SRGuildsAndKingdoms.src.guilds.LandClaim.ChunkSize + (SRGuildsAndKingdoms.src.guilds.LandClaim.ChunkSize / 2);
            int blockZ = chunkZ * SRGuildsAndKingdoms.src.guilds.LandClaim.ChunkSize + (SRGuildsAndKingdoms.src.guilds.LandClaim.ChunkSize / 2);
        ...
        private void OnNodeWarDataRequestReceived(IServerPlayer player, NodeWarDataRequestPacket packet)
        {
            ...
            var pvpMod = serverApi.ModLoader.GetModSystem("SRGuildsAndKingdomsPVP.PVPModSystem");
            if (pvpMod != null)
            {
